...
#include <math.h>
//#include <unistd.h> //keep this for linux based systems
#include <windows.h> //keep this for windows
...
{
	//comment following lines to work on windows
	//int argc;
	//char **argv;
	//
	//    argc = 0;
...
void display_function()
{
	...
	if (global.current_frame<global.N_frames - speed)
	{
		global.current_frame += speed;
		printf("%d/%d\n", global.current_frame, global.N_frames);
		fflush(stdout);
	}
	else global.current_frame = 0;

	//usleep(300000); //microseconds for unix
	Sleep(300); //milliseconds for Windows
}
